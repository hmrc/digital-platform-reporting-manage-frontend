...
class CanPhoneIndividualControllerSpec extends SpecBase with MockitoSugar {
  ...
  private lazy val canPhoneIndividualRoute = routes.CanPhoneIndividualController.onPageLoad.url
  ...
  "CanPhoneIndividual Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(CanPhoneIndividualPage, true).success.value
    ...
    "must update the subscription, save user answers, send an audit event, and redirect to the next page when the answer is no" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, canPhoneIndividualRoute)
            .withFormUrlEncodedBody(("value", "false"))

        val expectedContact = IndividualContact(Individual("first", "last"), "foo@example.com", None)
