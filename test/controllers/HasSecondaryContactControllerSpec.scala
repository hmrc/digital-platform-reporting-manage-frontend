...
class HasSecondaryContactControllerSpec extends SpecBase with MockitoSugar {
  ...
  private lazy val hasSecondaryContactRoute = routes.HasSecondaryContactController.onPageLoad.url
  ...
  "HasSecondaryContact Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(true), contactName)(using request, messages(application)).toString
      }
    }

    "must update the subscription, save user answers, audit the evenr, and redirect to the next page when the answer is no" in {
      ...
      val originalSecondaryContact = OrganisationContact(Organisation("second name"), "bar@example.com", Some("07777 888888"))
      ...
      running(application) {
        ...
        savedAnswers.get(SecondaryContactNamePage) must not be defined
        savedAnswers.get(SecondaryContactEmailAddressPage) must not be defined
        savedAnswers.get(CanPhoneSecondaryContactPage) must not be defined
