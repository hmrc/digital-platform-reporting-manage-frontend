...
class CanPhonePrimaryContactControllerSpec extends SpecBase with MockitoSugar {
  ...
  val formProvider = new CanPhonePrimaryContactFormProvider()
  val form = formProvider()

  lazy val canPhonePrimaryContactRoute = routes.CanPhonePrimaryContactController.onPageLoad(NormalMode).url
  ...
  "CanPhonePrimaryContact Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(CanPhonePrimaryContactPage, true).success.value
